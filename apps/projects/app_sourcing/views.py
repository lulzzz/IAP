...
# RPCs
from analytics.execution.lifung_sourcing.lifung_siamesenn_test import image_similarity_match
    ...
    def get(self, request, format=None, **kwargs):
        ...
        query_list_of_dict = [
            ...
            },
            {
                ...
                'model': models.DimProduct,
                'search_field_label': 'silhouette',
                'search_field': 'silhouette',
                'search_type': 'icontains',
                'is_unique': False,
            },
            {
            ...
            },
            {
                ...
                'search_field_label': 'product type',
                'search_field': 'name',
                'search_type': 'icontains',
                'is_unique': False,
            },
            ...
            },
            {
                'search_tab': 'search_tab_one_vendor', # only for multiple
                'model': models.DimVendor,
                'search_field_label': 'vendor code',
                'search_field': 'code',
                'search_type': 'icontains',
                'is_unique': True,
            },
            {
                ...
                'search_field_label': 'vendor country',
                'search_field': 'country',
                'search_type': 'icontains',
                'is_unique': False,
            },
            {
                'search_tab': 'search_tab_one_sample', # only for multiple
                'model': models.DimSample,
                'search_field_label': 'test report',
                'search_field': 'report_file_name',
                'search_type': 'icontains',
                'is_unique': True,
            },
            {
                'search_tab': 'search_tab_multiple_sample', # only for multiple
                'model': models.DimSample,
                'search_field_label': 'sample style',
                'search_field': 'style',
                'search_type': 'icontains',
                'is_unique': False,
            },
            {
                'search_tab': 'search_tab_multiple_sample', # only for multiple
                'model': models.DimSample,
                'search_field_label': 'sample article number',
                'search_field': 'article_number',
                'search_type': 'icontains',
                'is_unique': False,
            },
            {
                'search_tab': 'search_tab_multiple_sample', # only for multiple
                'model': models.DimSample,
                'search_field_label': 'sample vendor',
                'search_field': 'dim_vendor',
                'search_type': 'icontains',
                'is_unique': False,
            },
            {
                'search_tab': 'search_tab_multiple_sample', # only for multiple
                'model': models.DimSample,
                'search_field_label': 'sample description',
                'search_field': 'sample_description',
                'search_type': 'icontains',
                'is_unique': False,
            },
            {
                'search_tab': 'search_tab_multiple_sample', # only for multiple
                'model': models.DimSample,
                'search_field_label': 'sample color',
                'search_field': 'color',
                'search_type': 'icontains',
                'is_unique': False,
            },
...
class ProductAllocationMapAPI(views.WorldMap):
    ...
    def get(self, request, format=None, **kwargs):
        label_list = sorted(list(set(models.FactAllocation.objects.values_list('dim_vendor__dim_location__country_code_a2', flat=True))), key=str.lower)
        label_list = [x.lower() for x in label_list]
        data_dict = dict()
        for label in label_list:
    ...
    def set_filter_dict(self):
        ...
        if query.exists():
            ...
            if not models.FactAllocationAdHoc.objects.filter(uploaded_file_id=uploaded_file.id).exists():
                image_similarity_match(file_path=abs_path, uploaded_file_id=uploaded_file.id, limit=50)
